...
if __name__ == "__main__":
    ...
    try:
        ...
        # Create all comparison analytics tables
        ...
        summary = comparison.get_comparison_summary("birth_data")
        print("\n" + "=" * 80)
        print("COMPARISON SUMMARY")
        print("=" * 80)
        print(f"JAN Total Records: {summary.get('jan_total_records')}")
        print(f"JAN Unique Names: {summary.get('jan_unique_names')}")
        print(f"JAN Top 80% Names: {summary.get('jan_top80_count')}")
        print(f"\nAPR Total Records: {summary.get('apr_total_records')}")
        print(f"APR Unique Names: {summary.get('apr_unique_names')}")
        print(f"APR Top 80% Names: {summary.get('apr_top80_count')}")
        print(f"\nCommon Names: {summary.get('common_names_count')}")
        print(f"Unique to JAN: {summary.get('unique_jan_names_count')}")
        print(f"Unique to APR: {summary.get('unique_apr_names_count')}")
        print(f"\nJAN Top 80% in APR: {summary.get('jan_top80_in_apr_count')} ({summary.get('jan_top80_in_apr_pct')}%)")
        print(f"APR Top 80% in JAN: {summary.get('apr_top80_in_jan_count')} ({summary.get('apr_top80_in_jan_pct')}%)")
        print(f"Both Top 80%: {summary.get('both_top80_count')}")
        
        # Get common names (first page)
        common_names, total = comparison.get_common_names("birth_data", page=1, per_page=10)
        print(f"\nCommon Names (showing 10 of {total}):")
        for name in common_names[:5]:
            print(f"  {name['firstname']}: JAN={name['jan_frequency']}, APR={name['apr_frequency']}")
        
        # Export to CSV
        comparison.export_common_names_to_csv("birth_data", "/home/claude/common_names.csv")
        comparison.export_unique_jan_to_csv("birth_data", "/home/claude/unique_jan.csv")
        comparison.export_unique_apr_to_csv("birth_data", "/home/claude/unique_apr.csv")
